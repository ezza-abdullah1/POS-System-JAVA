    ...
    public double processMetroCardPayment(String cardNumber, double amount) throws SQLException {
        try (Connection conn = dbConnection.getConnection()) {
            ...
            try {
                ...
                if (cardResult.next()) {
                    int points = cardResult.getInt("points");
                    double deduction = Math.min(points, amount);

                    String updatePointsQuery = "UPDATE MetroCard SET points = points - ? WHERE metCrdNum = ?";
                    PreparedStatement updateStmt = conn.prepareStatement(updatePointsQuery);
                    updateStmt.setInt(1, (int) deduction);
                    updateStmt.setString(2, cardNumber);
                    updateStmt.executeUpdate();
